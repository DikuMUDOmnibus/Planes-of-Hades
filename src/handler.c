...
work on this */
	...
	29600 || obj->pIndexData->vnum == 29601 || obj->pIndexData->vnum ==
	29602 || obj->pIndexData->vnum == 29603 || obj->pIndexData->vnum ==
        29604 || obj->pIndexData->vnum == 29605 || obj->pIndexData->vnum ==
        29606 || obj->pIndexData->vnum == 29607 || obj->pIndexData->vnum ==
        29608 || obj->pIndexData->vnum == 29609 || obj->pIndexData->vnum ==
        29610 || obj->pIndexData->vnum == 29611 || obj->pIndexData->vnum ==
        29612 || obj->pIndexData->vnum == 29613 || obj->pIndexData->vnum ==
        29614 || obj->pIndexData->vnum == 29615 || obj->pIndexData->vnum ==
        29616)

        if (IS_NPC(ch) || !IS_CLASS(ch, CLASS_DROW) || IS_CLASS(ch, CLASS_MONK)){
	act( "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is zapped by $p and drops it.",  ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
	obj_to_room( obj, ch->in_room );
...
!IS_CLASS(ch,CLASS_NINJA))) {
  ...
  CLASS_NINJA)) && IS_SET(obj->spectype, SITEM_NINJA))
    {

	act( "You are scared of $p and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is scared of $p and drops it.",  ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
	obj_to_room( obj, ch->in_room );
	do_autosave( ch, "" );
	return;
	...
	if ((IS_NPC(ch) || !IS_CLASS(ch,
     CLASS_MAGE)) && IS_SET(obj->spectype, SITEM_MAGE))
    {
	act( "You are burned by $p and drop it.", ch, obj, NULL, TO_CHAR );
...
&&  !IS_SET(obj->spectype,SITEM_DEMONIC) && !IS_SET(obj->spectype,SITEM_NINJA)
 ...
 */
	...
	else if (sn == 45 || sn == 1)
	{
	    ...
	    send_to_char( "Your eyes glow #1red#n.\n\r", ch );
	...
	else if (sn == 57 || sn == 8)
	{
	    ...
	    send_to_char( "You are surrounded by a #7white aura#n.\n\r", ch );
	    act("$n is surrounded by a #7white aura.#n",ch,NULL,NULL,TO_ROOM);
	}
	else if (sn == 9)
	{
	...
	else if (sn == 22)
	{
	    ...
	    send_to_char( "Your eyes begin to glow bright #7white.#n\n\r", ch );
	    act("$n's eyes begin to glow bright #7white.#n",ch,NULL,NULL,TO_ROOM);
	}
	else if (sn == 23)
...
void unequip_char( CHAR_DATA *ch, OBJ_DATA *obj )
{
   ...
   || IS_SET(obj->spectype, SITEM_MAGE)
		...
		&& !is_affected(ch, 45))
	{
	    ...
	    send_to_char( "Your eyes stop glowing #1red.#n\n\r", ch );
	    act("$n's eyes stop glowing #1red.#n",ch,NULL,NULL,TO_ROOM);
	}
		...
		&& !is_affected(ch, 57))
	{
	    ...
	    send_to_char( "The #7white aura#n around your body fades.\n\r", ch );
	    act("The #7white aura#n about $n's body fades.",ch,NULL,NULL,TO_ROOM);
	}
	...
	else if (IS_ITEMAFF(ch, ITEMA_VISION) && sn == 22)
	{
	    ...
	    send_to_char( "Your eyes stop glowing bright #7white.#n\n\r", ch );
	    act("$n's eyes stop glowing bright #7white.#n",ch,NULL,NULL,TO_ROOM);
	}
	else if (sn == 23)
