...
#define SITEM_MAGE      268435456
#define SITEM_NINJA     536870912
/*
...
#define NEW_PCLAWS    16777216 // Poison Claws
...
#define IS_CREATOR(ch)		(!str_cmp(GET_PC_NAME(ch), "Dunkirk" ))
...
#define CAN_PK(ch)		(get_trust(ch) > 2 && get_trust(ch) < 13)
...
DECLARE_DO_FUN( do_pclaws        ); //poison claws by Aronacus
...
DECLARE_DO_FUN(   do_mageq     );      /* mage eq by Aronacus */
// DECLARE_DO_FUN(   do_monkeq   );      /* monk eq command  by Aronacus */
...
DECLARE_DO_FUN(  do_ninjagear);    /* Ninja eq command */
...
DECLARE_SPELL_FUN(	spell_phantoms_shade	);
DECLARE_SPELL_FUN(	spell_aura_of_mistriss  );
...
#define BACKUP_DIR      "../player/backupInfo/" /*reimb shit*/
