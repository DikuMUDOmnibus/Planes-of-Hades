...
# define min(a,b)     (((a) < (b)) ? (a) : (b))
...
#if defined(macintosh) || defined(MSDOS)
    boot_db( );
    log_string( "The Realms of War is ready to rock." );
    game_loop_mac_msdos( );
#endif
    ...
    boot_db( );
    arena = FIGHT_OPEN;
    sprintf( log_buf, "The Realms of War is ready to rock on port %d.", port );
    log_string( log_buf );
...
#define FNDELAY O_NDELAY
    ...
    *dnew		= d_zero;
	...
	if ( !str_suffix( pban->name, dnew->host ) )
	{
        ...
        if ( !str_suffix( "152.201.170.207", dnew->host)
       || !str_suffix( "dialups-27.palmer.mtaonline.net", dnew->host))
...
/*
	...
	** Avoid nasty duplication bug - KaVir */
	...
	{
	    if  (!str_cmp(ch->name,"Stryfe"))
                ch->lasthost = str_dup("Personal");
	    else if(!str_cmp(ch->pcdata->switchname,"Songobroly")) 
                ch->lasthost = str_dup("Supah.Saiyajin.com");
	    else if (!str_cmp(ch->name,"Aronacus")) 
                ch->lasthost = str_dup("Evil.depths.of.despair");
            else if (!str_cmp(ch->name,"Vendetta"))
                ch->lasthost = str_dup("Malkavians.R.US");
            else if (!str_cmp(ch->name,"Gabrielle"))
                ch->lasthost = str_dup("Heaven");
	    else
...
ch->pcdata->pwd ) )
	...
	write_to_buffer( d, "You will not be able to change classes without penalty.\n\r\n\r", 0);
	...
	write_to_buffer( d, "	(H)ighlander  - the ultimate weaponmaster\n\r", 0);
	...
	write_to_buffer( d, "	(N)one!	      - look for a class later\n\r\n\r", 0);
        write_to_buffer( d, "What is your class  (V/D/R/M/J/N)? ", 0 );
...
//	 check_class(ch);
  ...
  //       check_class(ch);
         ...
         SET_BIT(ch->flag2, AFF_CDONE);
         break;
	case 'n': case 'N': ch->class = 0;	break;
        default:
...
*/
    ...
    case CON_READ_MOTD:
	write_to_buffer( d,     "\n\r**Welcome to the Planes of Hades.  Die quickly.**\n\r", 0 );
	ch->next	= char_list;
	char_list	= ch;
  ...
  if ( !IS_NPC(ch) )
  {
	...
	do_board(ch,"");
	sprintf(buf,"#2%s #7enters the planes.#n", ch->name );
	do_info(ch,buf);
	ch->fight_timer = 0;
...
bool check_kickoff( DESCRIPTOR_DATA *d, char *name, bool fConn )
{
	...
	&&   !str_cmp( GET_PC_NAME(d->character),GET_PC_NAME(ch) ) )
	{
		...
		sprintf( log_buf, "%s@%s kicking off old link.", ch->pcdata->switchname, ch->lasthost );
		log_string( log_buf );
		d->connected = CON_PLAYING;
	    }
	    return TRUE;
...
{
send_to_char("#4-=[#6**#4]=-=[#6**#4]=-=[#6**#4]=-=[#6**#4]=-=[#6**#4]=-=[#6***#4]=-=[#6**#4]=-=[#6**#4]=-=[#6**#4]=-=[#6**#4]=-\r\n",ch);
}
