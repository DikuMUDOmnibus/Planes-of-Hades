   ...
   void do_mageq( CHAR_DATA *ch, char *argument )
{
    ...
    if      (!str_cmp(arg,"girth"     )) vnum = 2651;
    else if (!str_cmp(arg,"mask"   )) vnum = 2650;
    ...
    if (ch->practice < 60)
    {
	send_to_char("It costs 60 primal to create a piece of mage armor\n\r",ch);
	return;
    }
