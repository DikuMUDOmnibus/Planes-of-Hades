...
void check_muddeath(void)
{
    ...
    if (pulse_muddeath > 0)
    {
	sprintf(buf,"%s #1in %d minute(s)#n\n\r", isreboot ? "#1Reboot#n" : "#1Shutdown#n", pulse_muddeath);
...
|| 
	   ...
	   ||   ch->in_room->vnum == 32000
	   ||   ch->in_room->vnum == 30010
	   ||   ch->in_room->vnum == 32001
	   ||   ch->in_room->vnum == 32002 ) )
...
void weather_update( void )
{
	...
	strcat( buf, "The moon slowly sets. \n\r" );
	break;	
	...
	strcat( buf, "The moon rises to the east. \n\r" );
	break;
	...
	for ( d = descriptor_list; d != NULL; d = d->next )
	{
		...
		if (IS_EXTRA(ch, EXTRA_ROT))
		    ...
		    if (char_up) send_to_char( "You feel the power of Gaia heal your mangled body.\n\r", ch );
				...
				if ( IS_CLASS(ch, CLASS_VAMPIRE) )
		{
			...
			send_to_char("You become one generation closer to Cain!\n\r",ch);
			ch->generation -=1;
			ch->pcdata->gentick=0;
...
*/
     ...
     if ( IS_AFFECTED(ch, AFF_FLAMING) && !is_obj && !drop_out && ch->in_room != NULL )
	{
	    ...
	    (!room_is_dark(ch->in_room)) &&(weather_info.sunlight = SUN_LIGHT) && 
		(weather_info.sunlight = SUN_SET) && (weather_info.sunlight = SUN_RISE) )
	{
