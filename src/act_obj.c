...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
	...
	( obj->questowner != NULL && strlen(obj->questowner) > 1 && str_cmp(ch->pcdata->switchname,obj->questowner)))
    {
    ...
    if (obj->value[0] > 0 && !IS_NPC(ch) && obj->item_type != ITEM_PAGE 
        && IS_CLASS(ch, CLASS_DEMON) && IS_SET(obj->spectype, SITEM_DEMONIC) )
    {
	sprintf( buf, "You receive a refund of %d demon points from $p.", obj->points);
	act( buf, ch, obj, NULL, TO_CHAR );
	ch->pcdata->stats[DEMON_CURRENT] += obj->value[0];
    }
    extract_obj( obj );
