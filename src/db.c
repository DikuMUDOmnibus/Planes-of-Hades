...
void boot_db( void )
{
	...
	time_info.year	= lmonth / 17;
	     ...
	     if ( time_info.hour <  4 ) weather_info.sunlight = MOON_SET;
	else if ( time_info.hour <  5 ) weather_info.sunlight = SUN_DARK;
	else if ( time_info.hour <  6 ) weather_info.sunlight = SUN_RISE;
	else if ( time_info.hour < 13 ) weather_info.sunlight = SUN_LIGHT;
	else if ( time_info.hour < 15 ) weather_info.sunlight = SUN_SET;
	else if ( time_info.hour < 16 ) weather_info.sunlight = SUN_DARK;
	else if ( time_info.hour < 17 ) weather_info.sunlight = MOON_RISE;
	else							weather_info.sunlight = MOON_LIGHT;
	...
	fclose( fpList );
       ...
       log_string("Finished Loading Areas");
    /*
     * Fix up exits.
...
OBJ_DATA *create_object( OBJ_INDEX_DATA *pObjIndex, int level )
{
    ...
    *obj		= obj_zero;
    ...
    SET_BIT(obj->quest, QUEST_RELIC);
        obj->condition  = 100;
        obj->toughness  = 100;
        obj->resistance = 1;	}            /* mage eq */     else if (obj->pIndexData->vnum >= 2650
             && obj->pIndexData->vnum <= 2653
    ...
    SET_BIT(obj->quest, QUEST_RELIC);
        obj->condition  = 100;
        obj->toughness  = 100;
        obj->resistance = 1;	}   		/* ninja eq */    else if (obj->pIndexData->vnum >= 29724
             && obj->pIndexData->vnum <= 29733	 
	...
	obj->condition  = 100;
        obj->toughness  = 100;
        obj->resistance = 1;      }    else if (obj->pIndexData->vnum >= 27650 
