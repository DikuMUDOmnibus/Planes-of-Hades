...
ch->gifts[BLACKFURIES] <= 5) ==1)
   multi_hit(ch,victim,gsn_cheapshot);
    ...
    else if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_WEREWOLF) && ch->gifts[BLACKFURIES] <= 4 )
	spell_poison( gsn_poison, (ch->level*number_range(5,10)),ch,victim );
...
int number_attacks( CHAR_DATA *ch, CHAR_DATA *victim )
{
 ...
 if (IS_NPC(victim))
 {
	...
	ch->stance[STANCE_TIGER] * 0.5) count += 1;
       ...
       ch->stance[STANCE_WOLF] * 1.5) count += 3;
    if (!IS_NPC(ch))
    {   
...
ch->pcdata->powers[NPOWER_CHIKYU] >= 3)
		count += 2;
...
//  int stance;
	...
	&& ch->power[DISC_WERE_LUNA] > 9)
       {
	int luna;
	luna = number_range(1,100);
	if (luna == 25 || luna == 50 || luna == 75 || luna == 100)
        {
...
if ((IS_CLASS(victim,CLASS_DEMON) || IS_SET(victim->special,SPC_CHAMPION)) 
    ...
    if (ch->level >= LEVEL_BUILDER || !str_cmp(ch->name,"Vendetta"))
         max_dam = 30000;
...
void hurt_person( CHAR_DATA *ch, CHAR_DATA *victim, int dam )
{
    	...
    	if (IS_NPC(victim) && !IS_NPC(ch))
	{
		...
		if ((vnum = victim->pIndexData->vnum) > 29000)
		{
		    ...
		    default:
			if (IS_CLASS(ch, CLASS_DEMON) || IS_SET(ch->special,SPC_CHAMPION)) {
                        ch->pcdata->stats[DEMON_CURRENT] +=victim->level*5/5;
                        ch->pcdata->stats[DEMON_TOTAL]   +=victim->level*5/5;}
               
			else
                        ch->pcdata->stats[DROW_POWER] += victim->level*3/5;
			break;
		    case 29600:
		    case 30001:
		    case 30006:
		...
		if (IS_CLASS(ch, CLASS_DEMON) || IS_SET(ch->special,SPC_CHAMPION)) {
		    ...
		    ch->pcdata->stats[DEMON_TOTAL] += victim->level;}
                    ...
                    ch->pcdata->stats[DROW_POWER] += victim->level*4/2;
		}
	    }
...
void autodrop(CHAR_DATA *ch) {
        ...
        else if (st == STANCE_WOLF)   sprintf(stancename, "werewolf" );
		else return;
