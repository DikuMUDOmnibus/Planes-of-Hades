...
"[Arena] %s (%d wins) (%d losses) Payoff odds %f\n\r",
 ...
 victim->pcdata->switchname,victim->pcdata->awins,victim->pcdata->alosses, odd1 );

sprintf(buf2,

"[Arena] %s (%d wins) (%d losses) Payoff odds %f\n\r", 

 ch->pcdata->switchname,ch->pcdata->awins,ch->pcdata->alosses, odd2 ); 
...
/*== make sure they have the cash ==*/
 ...
 if(wager > ch->exp)
  ...
  send_to_char("You don't have that much exps to wager!\n\r",ch); 
