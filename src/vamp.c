...
sprintf(buf, "A huge rabid dragon");
free_string( ch->morph );
...
void do_embrace(CHAR_DATA *ch, char *argument  )
{
    ...
    if (victim->position != POS_STUNNED && victim->position != POS_SLEEPING && victim->position != POS_DEAD && victim->master != ch)
    {
	...
	case 'S':
        ...
        case 'E':
         ...
         if (!str_cmp(word,"End"))
         {
          ...
          send_to_char("***Classes will be passed out again. -Zubrei\n\r",ch);
          ch->level = 3;
          ch->trust = 3;
          ch->exp += 250000;
	  do_autosave(ch,"");
