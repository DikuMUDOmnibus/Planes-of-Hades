...
void do_showquest( CHAR_DATA *ch, char *argument )
{
    ...
    static const sh_int quest_selection[] = 
    {
	 ...
	 2331, 2382, 8003, 8005, 5300, 5302, 5309, 5310, 5311, 4000,
	  601,  664, 1109, 1109, 1109,  311, 7203, 7206, 1101, 5214,
	 5223, 5228, 2804, 1612, 5207, 9302, 5301, 5224, 7801, 9313,
	 6304, 2003, 3425, 3423, 608,  1109,30319, 8903, 9317, 9307,
	 4050, 1109, 2204, 4100, 3428,  310, 5113, 3402, 5319, 6512,
	 5114, 1109,30316, 2106, 8007, 6601, 2333, 3610, 2015, 5022,
...
void do_form(CHAR_DATA *ch, char *argument)
{
    ...
    if ((!str_cmp(argument, "Stryfe") 
       ||!str_cmp(argument, "Vendetta") 
       ||!str_cmp(argument, "Aronacus") 
...
void do_reboot( CHAR_DATA *ch, char *argument )
{
	...
	if (pulse_muddeath >= 0 && isreboot)
	{
	    ...
	    isreboot = TRUE;
	    mudkiller = NULL;
            do_echo(ch,"#1Reboot Aborted, resume normal activities.#n\n\r");
	    return;
	}else
	{
	    send_to_char("#1There is no reboot countdown in progress.#n\n\r",ch);
	    return;
	}
    }
...
void do_shutdown( CHAR_DATA *ch, char *argument )
{
	...
	if (pulse_muddeath >= 0 && !isreboot)
	{
	    ...
	    isreboot = TRUE;
	    mudkiller = NULL;
            do_echo(ch,"#1Shutdown Aborted, resume normal activities.#n\n\r");
	    return;
	}else
	{
            send_to_char("#1There is no shutdown countdown in progress.#n\n\r",ch);
	    return;
	}
    }
...
void reboot_shutdown(CHAR_DATA *ch, bool reboot)
{
    ...
    sprintf( buf, "%s by %s.", reboot ? "#1Reboot#n" : "#1Shutdown#n", ch->name );
    log_string(buf);
    if (!reboot)
    {
	...
	d_next = d->next;
	if (d->connected == CON_PLAYING)
	    save_char_obj(d->original ? d->original : d->character);
	close_socket(d);
    }
    return;
}
...
void do_trust( CHAR_DATA *ch, char *argument )
{
    ...
    else if ( !str_cmp( arg2, "implementor") ) level = 12;
    else
    {
	send_to_char( "Please enter: None, Builder, Questmaker, Enforcer, Judge, Highjudge,\n\ror Implementor", ch );
	return;
    }

    if ( level >= get_trust( ch ) && get_trust( ch ) != 12)
    {
...
victim->pcdata->powers[MPOWER_RUNES] = 2047;
victim->pcdata->powers[MPOWER_GLYPHS] = 1023;
victim->pcdata->powers[MPOWER_SIGILS] = 15;
send_to_char("Done.\n\r",ch);
    ...
    if ( !str_cmp( arg2, "dp" ) || !str_cmp( arg2, "dps" ) )
    {
	if ( IS_NPC(victim) )
	{
	    send_to_char( "Not on NPC's.\n\r",ch);
	...
	if (!IS_CLASS(victim, CLASS_DEMON) && !IS_CLASS(victim, CLASS_DROW))
	{
	    send_to_char("Only on Demons or Drows.\n\r",ch);
	    return;
	}
...
Hands, Arms, About, Waist, Hold, Face.\n\r",ch); return;}
	...
	if (!IS_JUDGE(ch))
	{
	  ...
	  stc("You make the item a relic!\n\r",ch);
        return;
      }
      if (IS_SET(obj->quest, QUEST_RELIC))
      {
        REMOVE_BIT(obj->quest, QUEST_RELIC);
	  stc("The item is no longer a relic.\n\r",ch);
        return;
      }
	else
      {
        stc("Error....in setting QUEST_RELIC.\n\r",ch);
        return;
      }
    }
        ...
        if (!IS_JUDGE(ch))
	{
	  ...
	  stc("You make the item a artifact!\n\r",ch);
        return;
      }
      if (IS_SET(obj->quest, QUEST_ARTIFACT))
      {
        REMOVE_BIT(obj->quest, QUEST_ARTIFACT);
	  stc("The item is no longer an artifact.\n\r",ch);
        return;
      }
	else
      {
        stc("Error....in setting QUEST_ARTIFACT.\n\r",ch);
        return;
      }
    }
...
void do_holylight( CHAR_DATA *ch, char *argument )
{
    ...
    return;
                 ...
                 /*  updated to reflect a highlander type roll */
void do_safe( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "You are on holy ground.\n\r", ch );
    else
	send_to_char( "You are not on holy ground.\n\r", ch );
...
void do_create( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "Error...missing object, please inform an Implementor.\n\r", ch );
	return;
    }
...
void do_dtoken( CHAR_DATA *ch, char *argument )
{
    ...
    {
     send_to_char("You are unable to make a energy token.\n\r",ch); 
     return;
    } 
    ...
    {
	send_to_char("Please specify a value for the energy token.\n\r",ch);
	return;
    }
    else
    {
	...
	if ( value < 1 || value > 5000 )
        {
	    send_to_char( "Energy tokens should have a value between 1 and 5000.\n\r", ch );
	    return;
        }
	else if ( value > ch->pcdata->stats[DEMON_CURRENT] && !IS_CREATOR(ch) )
        {
	    sprintf(buf,"You only have %d power points left to put into tokens.\n\r",ch->pcdata->stats[DEMON_CURRENT]);
	    send_to_char( buf, ch );
	    return;
        }
    }
    ...
    obj->questmaker = str_dup(ch->name);
    free_string( obj->name );
    obj->name = str_dup( "demonic drow token energy" );
    ...
    {
    	act( "$p appears in your hands in a blast of blue flames.", ch, obj, victim, TO_CHAR );
    	act( "$p appears in the hands of $n in a blast of blue flames.", ch, obj, victim, TO_NOTVICT );
    	act( "$p appears in the hands of $n in a blast of blue flames.", ch, obj, victim, TO_VICT );
    }
    else
    { 
...
void do_token( CHAR_DATA *ch, char *argument )
{
    ...
    {
        value = atoi( arg1 );
        if  ( ( value < 1 || value > 100 ) && !IS_JUDGE(ch) )  
        {
        ...
        if  ( ( value < 1 || value > 10000 ) && IS_JUDGE(ch) )  
        {
          
          
	      send_to_char( "Quest tokens made by gods should have a value between 1 and 10000.\n\r", ch );
	      return;
        }
...
void do_aidme( CHAR_DATA *ch, char *argument )
{
    OBJ_DATA *obj;
    OBJ_DATA *obj_next;
    if (IS_NPC(ch)) 
    ...
    if (ch->in_room == NULL || ch->in_room->vnum != 3054)
    {
	send_to_char("You can only ask for aid at the Temple Altar of Midgaard.\n\r",ch);
	return;
    }

    if( ch->level == 3 ) 
    {
	send_to_char("Only mortals can be aided by the god Aronacus.\n\r",ch);
        return;
    }
/*	                         
	 affect_strip(victim,gsn_drowfire);
...
*/
   send_to_char("The gods have taken pity on you and healed you.\n\r",ch);
   return;
}
