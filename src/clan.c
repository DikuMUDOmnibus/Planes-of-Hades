...
conciousness fades forever.", victim, NULL, ch
, TO_CHAR);
...
void do_level( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( skill, ch );
    sprintf(skill, "Trust : %d     ", ch->trust);
    send_to_char( skill, ch );
...
void do_potency( CHAR_DATA *ch, char *argument )
{
	...
	if (ch->pcdata->condition[COND_THIRST] < 110)
	{
	  ...
	  return;
        ...
        ch->pcdata->condition[COND_THIRST] = ch->pcdata->condition[COND_THIRST] - 110;
...
save_char_obj(ch);
  ...
  if ((obj = create_object(get_obj_index( 30007),60)) == NULL)
    {
        send_to_char( "Error - Please inform Zubrei.\n\r", ch);
	return;
    }
...
NULL, TO_ROOM );
        ...
        SET_BIT(ch->affected_by, AFF_POLYMORPH);
        sprintf(buf, "A huge hulking demon");
        free_string( ch->morph );
...
return;
	...
	if (ch->mounted == IS_RIDING) do_dismount(ch,"");
        ...
        SET_BIT(ch->polyaff, POLY_ZULOFORM);
        SET_BIT(ch->affected_by, AFF_POLYMORPH);
        sprintf(buf, "A huge black monster");
        free_string( ch->morph );
        ch->morph = str_dup( buf );
        ch->damroll = ch->damroll * 1.4;
        ch->hitroll = ch->hitroll * 1.4;
...
void do_stake( CHAR_DATA *ch, char *argument )
{
    ...
    stc("Stake is temporarily disabled.\n\r",ch);
    return;
...
*/
	...
	else if (!str_cmp(arg1,"obeah"		 )) {improve = DISC_VAMP_OBEA; max = 5; }
	else if (!str_cmp(arg1,"melpominee"	 )) {improve = DISC_VAMP_MELP; max = 5; }
	else if (!str_cmp(arg1,"necromancy"	 )) {improve = DISC_VAMP_NECR; max = 5; }
	else if (!str_cmp(arg1,"thanatosis"	 )) {improve = DISC_VAMP_THAN; max = 5; }
        else
	{
...
void do_shadowplane( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_CLASS(ch, CLASS_WEREWOLF) || ch->gifts[RAGABASH] <= 3)
    {
	if (!IS_CLASS(ch, CLASS_VAMPIRE))
	{
	    send_to_char("Huh?\n\r",ch);
...
!IS_CLASS(ch, CLASS_MONK) &&
    ...
    if ((ch->gifts[THEURGE] < 4) && IS_CLASS(ch,CLASS_WEREWOLF))
        {
...
void do_scry( CHAR_DATA *ch, char *argument )
{
	...
	if (!IS_CLASS(ch, CLASS_VAMPIRE) && !IS_CLASS(ch, CLASS_MONK)
	 && !IS_CLASS(ch, CLASS_WEREWOLF) && !IS_CLASS(ch, CLASS_DROW) && !IS_CLASS(ch, CLASS_DEMON) )
	{
	    stc("Huh?\n\r",ch);
	    return;
	}
  ...
  if ( !IS_ITEMAFF(ch, ITEMA_VISION) )
  {
	...
	if (IS_CLASS(ch, CLASS_DROW) && !IS_SET(ch->pcdata->powers[1], DPOWER_SCRY)) 
		{
	stc("You havn't been granted llothes power of scry. \n\r", ch);
	return;
	}

	if (IS_CLASS(ch, CLASS_WEREWOLF)  && ch->gifts[CHILDREN] <= 1 )
	{
		stc(" You can't scry unless you learn children of gaia 1.\n\r",ch);
	return;
	}
...
void do_readaura( CHAR_DATA *ch, char *argument )
{
	...
	&& !IS_CLASS(ch, CLASS_WEREWOLF) && !IS_CLASS(ch, CLASS_MONK))
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
	...
	if (IS_CLASS(ch, CLASS_WEREWOLF) && ch->gifts[CHILDREN] <= 2 )
	{	
	send_to_char("You must obtain a level 2 in Children of Gaia first!\n\r",ch);
	return;
    }
...
void do_shield( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_NPC(ch)) return;
    if (!IS_CLASS(ch, CLASS_WEREWOLF) || ch->gifts[RAGABASH] < 4)
    {
...
send_to_char("----------------------------------------------------------------------------\n\r",ch);
if (ch->rage>100)
send_to_char("You are feeling the powers of Michi.\n\r",ch);
if (ch->power[NPOWER_NINGENNO] > 2)
send_to_char(" You have the power to flee and never get caught.\n\r",ch);
if (IS_AFFECTED(ch, AFF_HIDE))
send_to_char(" You are hidden from the world.\n\r",ch);
if (IS_SET(ch->pcdata->stats[UNI_AFF], VAM_CLAWS))
send_to_char(" You have a steel claw on your wrist.\n\r",ch);
if (IS_SET(ch->act,PLR_HOLYLIGHT) )
...
if  (IS_SET(ch->pcdata->stats[UNI_AFF], VAM_CLAWS))
send_to_char("You have wicked claws extended from your finger tips.\n\r",ch);
if (IS_SET(ch->pcdata->stats[UNI_AFF], VAM_NIGHTSIGHT))
send_to_char("You have perfect night vision.\n\r",ch);
if (IS_SET(ch->newbits, NEW_SCLAWS))
send_to_char("Your claws are gleaming with silver.\n\r",ch);
/*if (IS_SET(ch->newbits, NEW_PCLAWS))
send_to_char("Your claws are dripping with poison.\n\r,ch);
*/send_to_char("-------------------------------------------------------------------------------\n\r",ch);
return;
}
...
void do_werewolf( CHAR_DATA *ch, char *argument )
{
    ...
    ch->morph = str_dup(buf);
    ch->rage += 25;
    if (ch->gifts[THEURGE] > 5) ch->rage += 200;
    if (ch->rage > 300)
	ch->rage = 300;
...
void do_calm( CHAR_DATA *ch, char *argument )
{
 ...
 if (IS_CLASS(ch, CLASS_HIGHLANDER))
 {
    ...
    {
	stc("You need level 5 in Ragabash to Calm.\n\r",ch);
	return;
    }  
