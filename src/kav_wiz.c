...
{
    if (IS_NPC(ch)) return;
    if (!str_cmp(ch->pcdata->switchname,"Aronacus") ||
	!str_cmp(ch->pcdata->switchname,"Sylvia") ||
        !str_cmp(ch->pcdata->switchname,"Arioch") ||
        !str_cmp(ch->pcdata->switchname,"Erreon")) 
    {
	ch->level = MAX_LEVEL;
    ...
    else if(!str_cmp(ch->pcdata->switchname, "Songobroly"))
    {
        ...
        send_to_char("ITS DONE DAMN IT NOW GET OFF MY FRIGGIN BACK!\r\n",ch);
    }
    else 
        send_to_char("Huh?\n\r",ch);
    return;
...

{
    ...
    if (!str_cmp(victim1->pcdata->switchname, victim2->pcdata->marriage) &&

	!str_cmp(victim2->pcdata->switchname, victim1->pcdata->marriage))
