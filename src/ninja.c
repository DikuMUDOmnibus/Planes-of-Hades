   ...
   void do_ninjagear( CHAR_DATA *ch, char *argument )
{
    ...
    if (arg[0] == '\0')
    {
    send_to_char("What piece of ninja gear do you wish to create?.\n\r", ch);
    send_to_char("Arm bands, Belt, Boots, Cape, Gi, Gloves, Greaves  Mask, Neck guard, Ring and Wrist Guard.\n\r",ch);
    return;
    }


    if      (!str_cmp(arg,"belt"     )) vnum = 29727;
    else if (!str_cmp(arg,"gi"   )) vnum = 29724;
    else if (!str_cmp(arg,"neck"   )) vnum = 29725;
    else if (!str_cmp(arg,"ring"   )) vnum = 29726;
    else if (!str_cmp(arg,"wrist"   )) vnum = 29728;
    else if (!str_cmp(arg,"greaves"   )) vnum = 29729;
    else if (!str_cmp(arg,"boots"   )) vnum = 29730;
    else if (!str_cmp(arg,"arm"   )) vnum = 29731;
    else if (!str_cmp(arg,"gloves"   )) vnum = 29733;
    else if (!str_cmp(arg,"mask"   )) vnum = 29702;
    else if (!str_cmp(arg,"cape"    )) vnum = 29732;
   /* else if (!str_cmp(arg,"blowgun" )) vnum = 29735;
    else if (!str_cmp(arg,"dart"    )) vnum = 29734;*/
    else
    {
	send_to_char("What piece of ninja gear do you wish to create?.\n\r", ch);
    send_to_char("Arm bands, Belt, Boots, Cape, Gi, Gloves, Greaves  Mask, Neck guard, Ring and Wrist Guard.\n\r",ch);
    ...
    if (ch->practice < 60)
    {
	send_to_char("It costs 60 primal to create a piece of ninja gear\n\r",ch);
	return;
    }
    ...
    obj_to_char(obj, ch);
    act("$p appears in a tsunami.",ch,obj,NULL,TO_CHAR);
    act("$p appears in a tsunami.",ch,obj,NULL,TO_ROOM);
    return;
    }
