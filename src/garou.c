...
void do_gifts( CHAR_DATA *ch, char *argument )
{
	...
	stc("                                     #1Breed#n\n\r",ch);
	stc(lin, ch);
	sprintf(buf,"       #2Homid     [ %d ]         Metis    [ %d ]         Lupus       [ %d ]#n \n\r",
			ch->gifts[HOMID], ch->gifts[METIS], ch->gifts[LUPUS] );
	stc(buf, ch);
        stc(lin, ch);
	stc("                                    #3Auspice#n\n\r",ch);
	stc(lin, ch);
	sprintf(buf,"       #4Ragabash   [ %d ]        Theurge   [ %d ]        Philodox     [ %d ]#n\n\r",
		      ch->gifts[RAGABASH], ch->gifts[THEURGE], ch->gifts[PHILODOX] );
	stc(buf, ch);
	sprintf(buf,"      #5 Galliard   [ %d ]                                Ahroun      [ %d ]#n\n\r",
			ch->gifts[GALLIARD], ch->gifts[AHROUN] );
	stc(buf, ch);
	stc(lin, ch);
	  stc("                                     #8Tribe#n \n\r",ch);
	stc(lin, ch);															 
	sprintf(buf,"    #7Black Furies [ %d ]     Bone Gnawers [ %d ]    Children of Gaia [ %d ]#n\n\r",
