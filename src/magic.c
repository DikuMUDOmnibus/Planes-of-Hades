...
void spell_aura_of_mistriss( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
  ...
  * af.modifier  = 25;
  */affect_to_char( victim, &af );
    act( "$n is covered by the aura of Mistriss.", victim, NULL, NULL, TO_ROOM );
    send_to_char( "You are covered by Mistriss's Aura.\n\r", victim );
    return;
}


void spell_phantoms_shade( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    af.modifier  = 25;
    affect_to_char( victim, &af );
    act( "$n is covered by Phantom's Shade.", victim, NULL, NULL, TO_ROOM );
    send_to_char( "You are covered by Phantom's Shade.\n\r", victim );
    return;
}
...
void spell_identify( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( "This item is crafted from magical runes.\n\r", ch );
...
void spell_guardian( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    else if(get_trust(ch) == 12)
    {
       strcpy(buf,"Get your lazy self up here you lil demon!");
       do_say( ch, buf);
       send_to_char( "A demon bursts from the ground and cowers before you.\n\r",ch );
       act( "$N bursts from the ground and grubles about how $n treats him.", ch, NULL, victim, TO_ROOM );
    }
...
void spell_soulblade( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    free_string(obj->name);
    sprintf(buf,"%s soul %s",ch->pcdata->switchname,wpnname);
    obj->name=str_dup(buf);
    ...
    else sprintf(buf,"%s's soul %s is lying here.",ch->pcdata->switchname,wpnname);
    buf[0] = UPPER(buf[0]);
...
void spell_Gaiasblessing( int sn, int level, CHAR_DATA *ch, void *vo) {
	CHAR_DATA *victim = (CHAR_DATA *) vo;
	AFFECT_DATA af1;
	AFFECT_DATA af2;
	AFFECT_DATA af3;
	...
	act("A bright beam of moonlight strikes you.",ch,NULL,victim,TO_NOTVICT);
	act("A bright moon beam strikes $n .",ch,NULL,victim,TO_VICT);
	...
	send_to_char("You have been blessed by Gaia.\n\r", victim);
	act("$n is struck by a moon beam.",victim,NULL,NULL,TO_ROOM);
	return;
}
*/
