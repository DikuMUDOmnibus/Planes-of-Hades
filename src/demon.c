...
"-=(**)=-=(**)=-=(**)=-=(**)=-=(**)=-=(**)=-=(**)=-=(**)=-=(**)=-=(**)=-\n\r");
    send_to_char( lin,ch);
    send_to_char(
"                      #2-=#0(#6**#0)#2=- #6Attained Warps #2-=#0(#6**#0)#2=-#n\n\r",ch);
send_to_char(lin,ch);
...
void do_redeem( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char("You need 20000 demon points to redeem
warps!\n\r",ch);
	return;
    }
    {
...
void do_demonarmour( CHAR_DATA *ch, char *argument )
{
    ...
    if (arg[0] == '\0')
    {
    send_to_char("Command: demonarmor <black/red/brass> <piece>.\n\r", ch);
    return;
    }
    ...
    else if (!str_cmp(arg2,"ring"     ) && !str_cmp(arg,"brass")) vnum = 28650;
    else if (!str_cmp(arg2,"collar"   ) && !str_cmp(arg,"brass")) vnum = 28651;
    else if (!str_cmp(arg2,"plate"    ) && !str_cmp(arg,"brass")) vnum = 28652;
    else if (!str_cmp(arg2,"helmet"   ) && !str_cmp(arg,"brass")) vnum = 28653;
    else if (!str_cmp(arg2,"leggings" ) && !str_cmp(arg,"brass")) vnum = 28654;
    else if (!str_cmp(arg2,"boots"    ) && !str_cmp(arg,"brass")) vnum = 28655;
    else if (!str_cmp(arg2,"gauntlets") && !str_cmp(arg,"brass")) vnum = 28656;
    else if (!str_cmp(arg2,"sleeves"  ) && !str_cmp(arg,"brass")) vnum = 28657;
    else if (!str_cmp(arg2,"cape"     ) && !str_cmp(arg,"brass")) vnum = 28658;
    else if (!str_cmp(arg2,"belt"     ) && !str_cmp(arg,"brass")) vnum = 28659;
    else if (!str_cmp(arg2,"bracer"   ) && !str_cmp(arg,"brass")) vnum = 28660;
    else if (!str_cmp(arg2,"visor"    ) && !str_cmp(arg,"brass")) vnum = 28661;
    else
    {
	send_to_char("Please specify which piece of demon armor you wish to make: Ring Collar\n\rPlate Helmet Leggings Boots Gauntlets Sleeves Cape Belt Bracer Visor.\n\r",ch);
	return;
...
15000)
         && !str_cmp(arg,"red"))
    {
     send_to_char("It costs 15000 points of power to create a piece of red demon
armour.\n\r",ch);
     return;
    }
    else if ( (ch->pcdata->stats[DEMON_TOTAL] < 20000 ||
ch->pcdata->stats[DEMON_CURRENT] <
20000)
         && !str_cmp(arg,"brass"))
    {
     send_to_char("It costs 20000 points of power to create a piece of brass demon
armour.\n\r",ch);
     return;
    }
    ...
    {
	send_to_char("Missing object, please inform Stryfe.\n\r",ch);
	return;
    }
...
/*	send_to_char("Huh?\n\r",ch);
    ...
    if (!IS_DEMPOWER( ch, DEM_TAIL) && IS_CLASS(ch, CLASS_DEMON))
    {
