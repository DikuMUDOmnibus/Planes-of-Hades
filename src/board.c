...
{ "Changes",    "All New Changes",      0, 2,   "all", DEF_INCLUDE, 60, NULL, FALSE }
};
...
static void do_nremove (CHAR_DATA *ch, char *argument)
{
	...
	if (str_cmp(ch->pcdata->switchname,p->sender) || (get_trust(ch) < MAX_LEVEL))
	{
	...
	CHAR_DATA *ch = d->character;
		...
		switch (tolower(argument[0]))
		{
			...
			case 'p': /* post note */
				...
				/* remove AFK status */
				ch->pcdata->in_progress = NULL;					
				sprintf( buf, "New note from %s, on 
the %s board.\n\r", ch->pcdata->switchname,
ch->pcdata->board->short_name);
				do_info(ch, buf);
