...
char *	const	dir_name	[]		=
{
    "#1north#n", "#2east#n", "#3south#n", "#4west#n", "#5up#n", "#0down#n"
};
...
void do_recall( CHAR_DATA *ch, char *argument )
{
    ...
    act( "$n's body flickers with #2green#n energy.", ch, NULL, NULL, TO_ROOM );
    act( "Your body flickers with #2green#n energy.", ch, NULL, NULL, TO_CHAR );
...
void do_home( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_SET(ch->in_room->room_flags, ROOM_NO_RECALL) ||
	 IS_SET(ch->in_room->room_flags, ROOM_SAFE) && !IS_IMMORTAL(ch) )
    {
...
/*Vorador put this back in, with the cap. */
	if (!str_cmp(arg1,"mv") || !str_cmp(arg1,"move"))
 	...
 	arg2[0] = '\0';
	     ...
	     return;
	     } else if (ch->max_move + 50 > 25000) {
		send_to_char("You cannot train movement points over 25000, sorry.\r\n", ch); 
  	        return;
             } 
       else 
       {
...
arg2[0] = '\0';
   	  if (ch->exp < ch->max_mana * 50) {
            ...
            return;
            } else if (ch->max_mana + 50 >25000) {
               send_to_char("You cannot train mana points over 25000, sorry.\r\n", ch);
                return;
		} else {
...
arg2[0] = '\0';
	  if (ch->exp < ch->max_hit * 50)
	  {
	    ...
	    return;
	  } else if (ch->max_hit+50 > 25000) {
 		  send_to_char("You cannot train hit points over 25000, sorry.\r\n", ch);
                return;
		 } else {              
...
&& (ch->pcdata->rank == AGE_ELDER))
  {
	...
	if ( ch->max_mana      < 70000 )
	{
	   sprintf( buf, "Mana             - %d exp per 50 points.\n\r",(ch->max_mana *50) );
	   send_to_char( buf, ch );
	}
	if ( ch->max_move      < 70000 )
	{
    ...
    if ( (*pAbility >= 25000) && (!str_cmp( arg1, "hp")))
    {
        if (last) act( "Your $T is already at maximum.", ch, NULL, pOutput, TO_CHAR );
        return;
    }
    if ( (*pAbility >= 25000) && (!str_cmp( arg1, "mana")))
    {
	if (last) act( "Your $T is already at maximum.", ch, NULL, pOutput, TO_CHAR );
	return;
    }
    if ( (*pAbility >= 25000) && (!str_cmp( arg1, "move")))
    {
	if (last) act( "Your $T is already at maximum.", ch, NULL, pOutput, TO_CHAR );
	return;
    }
